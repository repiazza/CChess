...
#include <trace.h>
   ...
   * Set 
   */
  int iRECT_SetDimensions( SDL_Rect *pSDL_Rect, SDL_Rect pst_RECT_Dimensions ){
    if ( pSDL_Rect == NULL )
      return FALSE;

    pSDL_Rect->x = pst_RECT_Dimensions.x;
    pSDL_Rect->y = pst_RECT_Dimensions.y;
    pSDL_Rect->w = pst_RECT_Dimensions.w;
    pSDL_Rect->h = pst_RECT_Dimensions.h;
    return TRUE;
  }
#if 0  
