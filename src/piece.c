...
PSTRUCT_PIECE pCreatePiece(char cType, uint8_t ui8Color)
{
  ...
  if (!pPiece)
  {
    vTraceError("Erro: Falha ao alocar memória para a peça.\n");
    return NULL;
  }
...
const char *pszGetPieceName(PSTRUCT_SQUARE pSquare)
{
  ...
  if (strcmp(pSquare->pszType, SQUARE_TYPE_PAWN_PIECE) == 0)
    return "P";
  if (strcmp(pSquare->pszType, SQUARE_TYPE_ROOK_PIECE) == 0)
    return "R";
  if (strcmp(pSquare->pszType, SQUARE_TYPE_KNIGHT_PIECE) == 0)
    return "N";
  if (strcmp(pSquare->pszType, SQUARE_TYPE_BISHOP_PIECE) == 0)
    return "B";
  if (strcmp(pSquare->pszType, SQUARE_TYPE_QUEEN_PIECE) == 0)
    return "Q";
  if (strcmp(pSquare->pszType, SQUARE_TYPE_KING_PIECE) == 0)
    return "K";
